...
const state = {
    ...
    midias:[],avaliacoes:[],watchlist:[],favoritos:[],seguidores:[],
...
if (localStorage.getItem("profile-fixture")==="yes") {
    state.midias = [1,2,3,4].map(id=>({id,tmdb_id:id,tipo:id===4?"tv":"movie",titulo:"Titulo "+id,poster_path:"/poster.jpg",data_lancamento:"2024-01-01"}));
    state.avaliacoes = [
...
storage:{from:()=>({
    ...
    rpc:async(name,input)=>{
        ...
        if(name==="cinepop_atualizar_perfil") { if(window.__failProfileSave)return result(null,{code:"23505"});
